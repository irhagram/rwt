    ...
    Combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
      ...
      if enteredZones then
        for i=1, #enteredZones do
          ...
          TriggerEvent("rwt:poly:masuk", enteredZones[i]["name"], enteredZones[i]["data"])
          TriggerServerEvent("rwt:poly:masuk", enteredZones[i]["name"], enteredZones[i]["data"])
        end
      end
      ...
      if leftZones then
        for i=1, #leftZones do
          ...
          TriggerEvent("rwt:poly:keluar", leftZones[i]["name"], leftZones[i]["data"])
          TriggerServerEvent("rwt:poly:keluar", leftZones[i]["name"], leftZones[i]["data"])
        end
      end
    end)
